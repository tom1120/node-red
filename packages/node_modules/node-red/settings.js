...
const app_root = projectRoot;
let runtime_file = '';
let filepath  = '';
if (environment === 'development') {
    // const user_dir = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH;
    // 当前环境文件
    filepath = app_root + '/.env.development';
    // 当前脚本所在目录
    runtime_file = path.resolve(__dirname, '.env.development');
} else if (environment === 'production') {
    // const app_root = process.env.APP_ROOT || '/app';
    filepath = app_root + '/.env.production';
    runtime_file = path.resolve(__dirname, '.env.production');
}

// 公有配置文件
const sharepath = app_root + '/.env';
const runtime_share_file = path.resolve(__dirname, '.env');
// 公共配置文件存在就加载
if (!fs.existsSync(runtime_share_file)) {
    fs.copyFileSync(sharepath, runtime_share_file);
}
if (fs.existsSync(runtime_share_file)) {
    require('dotenv').config({ path: runtime_share_file });
}
// 环境配置文件存在就加载
if (!fs.existsSync(runtime_file)) {
    fs.copyFileSync(filepath, runtime_file);
}
if (fs.existsSync(runtime_file)) {
    require('dotenv').config({ path: runtime_file });
}
...
module.exports = {
    ...
    adminAuth: {
        ...
        users: [{
            username: "admin",
            password: "$2b$08$GUNr26NvVdbrvzlyX9afm.fWqevKtrDqjq/xClhm9U3PH13mBLNUK",
            permissions: "*"
        }]
    },
     ...
     */
    httpNodeAuth: { user: "nr_qlong_interface", pass: "$2b$08$eaD49VwqkUz/PUkURFs73.OYoKP9PeaJfTsH/taVrriy85WmjwCkS" },
    ...
    editorTheme: {
        ...
        palette: {
            ...
            catalogues: [   // Alternative palette manager catalogues
                ...
                'http://192.168.96.18/catalogue.json'
            ],
        },
